...
class HttpHelper
{
    ...
    public static function curl($url, $httpMethod = "GET", $postFields = null, $headers = null)
    {
        ...
        $client = new \Swoole\Coroutine\Http\Client('dysmsapi.aliyuncs.com', 443, true);
        $client->setMethod($httpMethod);
        if (is_array($headers) && 0 < count($headers)) {
            $httpHeaders =self::getHttpHearders($headers);
            $client->setHeaders($httpHeaders);
        }
        $client->post('/', $postFields);

        $client->close();
        $httpResponse = new HttpResponse();
        $httpResponse->setBody($client->body);
        $httpResponse->setStatus($client->statusCode);
        return $httpResponse;
    }

    public static function curl_bak($url, $httpMethod = "GET", $postFields = null, $headers = null)
    {
        $ch = curl_init();
